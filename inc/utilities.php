...
/**
 * Utlities Class with various useful functions.
 * Most functions taken from: https://github.com/ngfw/Recipe
 *
 * TODO: add Namespace
...
class Helper {
    ...
    public static function validateEmail($address, $tempEmailAllowed = true) {
        ...
        if (filter_var($address, FILTER_VALIDATE_EMAIL) &&
            ...
            if ($tempEmailAllowed) {
                ...
                $handle = fopen(__DIR__.'/banned-domains.txt', 'r');
                $temp = [];
    ...
    public static function expandShortUrl( $shortURL ) {
        ...
        if ( isset($match) && is_array($match) && count($match) == 3 ) {
            ...
            $names = $match[1];
            $values = $match[2];
            if ( ( isset($originals) && isset($names) && isset($values) ) &&
    ...
    public static function makeClickableLinks( $string, $attributes = [] ) {
        return preg_replace( '@(https?://([-\w\.]+[-\w])+(:\d+)?(/([\w/_\.#-]*(\?\S+)?[^\.\s])?)?)@', '<a class="auto-link" href="$1" '.self::arrayToString($attributes).'>$1</a>', $string );
    }
    ...
    public static function debug($variable) {
        ...
        foreach ( $maps as $function => $pattern ) {
            $output = preg_replace_callback( $pattern, function ($matches) use ($function) {
                switch( $function ) {
                    ...
                    default:
                        return '<span style="color: #FF0000;">error</span>(<span style="color: #4D5D94;">no-class</span>)#error (<span style="color: #1287DB;">error</span>)';
        ...
        echo '<pre style="background-color: #CDDCF4;border: 1px solid #bbb;border-radius: 4px;font-size:14px;line-height:1.4;margin:30px;padding:7px">'.$header.$output.'</pre>';
    }
